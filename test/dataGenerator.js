...
import { InfluxDBDataGenerator as DataGenerator } from '../lib/data-generator.js';
...
test(`Test limit functions `, t => {
  ...
  const data = generator.dataFromMessage(message, dayjs());
  const allowedKeys = new Set(['mean', 'min', 'time']);
  for (const item of data) {
    const pointKeys = Object.keys(item.point);
    for (const key of pointKeys) {
      t.true(allowedKeys.has(key), `Unexpected key: [${key}] in point`);
    }
  }
});
